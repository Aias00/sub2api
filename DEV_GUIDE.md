...
| 项目 | 说明 |
|------|------|
| **上游仓库** | Wei-Shaw/cloudbase |
| **Fork 仓库** | bayma888/cloudbase-bmai |
...
| 数据库凭据 | user=`cloudbase`, password=`cloudbase`, dbname=`cloudbase` |
...
psql -U cloudbase -h 127.0.0.1 -d cloudbase -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER cloudbase WITH PASSWORD 'cloudbase';
...
```bash
# 连接数据库
psql -U cloudbase -h 127.0.0.1 -d cloudbase
...
# 执行 SQL 文件
psql -U cloudbase -h 127.0.0.1 -d cloudbase -f migration.sql
```
...
- [上游仓库](https://github.com/Wei-Shaw/cloudbase)
