...
Cloudbase は、複数の上流 AI アカウント、API キー、サブスクリプションリソース、業務機能を 1 つの管理サービスにまとめるためのセルフホスト型 AI ゲートウェイおよび業務運用プラットフォームです。リクエストルーティング、アカウントプール、ユーザーアクセス、課金、決済、クォータポリシー、可観測性、Worker 駆動の業務ワークフローを統合して扱えます。

## 業務機能

- **統合 AI ゲートウェイ** - OpenAI 互換、Claude、Gemini、Grok、Codex、Claude Code などの AI トラフィックを管理されたエンドポイントへルーティング
- **アカウントプール運用** - OAuth アカウント、API キー、プロキシ、アカウントグループ、優先度、スティッキーセッション、フェイルオーバーを一元管理
- **アクセスとクォータ制御** - ユーザー API キー、グループ、同時実行数、RPM/TPM、モデル権限、サブスクリプションプラン、プラットフォーム別クォータを設定
- **課金と決済** - トークン単位の利用量計算、残高とギフトクレジット、サブスクリプション、プロモコード、返金、アフィリエイト、セルフサービスチャージをサポート
- **決済プロバイダー管理** - EasyPay、Alipay、WeChat Pay、Stripe、Creem、Waffo を内蔵し、プロバイダーインスタンス、ルーティング、上限、Webhook、消込に対応（[設定ガイド](docs/PAYMENT.md)）
- **利用分析と監査** - リクエストログ、コスト内訳、トークン統計、ユーザー/アカウント帰属、監査ログ、運用トラブルシューティングデータを提供
- **運用モニタリング** - チャネルの健全性、レイテンシ、可用性、エラー透過、リスク制御、スケジューラ動作、Worker 状態を監視
- **Worker 駆動の業務機能** - WeChat 記事エクスポート、画像ワークスペース生成、ホットトピック/コンテンツジョブなどのオプション Worker を実行
