...
func (h *SettingHandler) GetSettings(c *gin.Context) {
	...
	payload := dto.SystemSettings{
		...
		WeChatExportFetchRetries:                            settings.WeChatExportFetchRetries,
		WeChatExportFetchTimeoutMS:                          settings.WeChatExportFetchTimeoutMS,
		WeChatExportWorkerConcurrency:                       settings.WeChatExportWorkerConcurrency,
		WeChatExportWorkerIntervalMS:                        settings.WeChatExportWorkerIntervalMS,
		WeChatExportWorkerLeaseSeconds:                      settings.WeChatExportWorkerLeaseSeconds,
		WeChatExportWorkerMaxBackoffMS:                      settings.WeChatExportWorkerMaxBackoffMS,
		ImageWorkspaceUpstreamURL:                           settings.ImageWorkspaceUpstreamURL,
		ImageWorkspaceGenerationTimeoutMS:                   settings.ImageWorkspaceGenerationTimeoutMS,
		ImageWorkspaceCompletionCost:                        settings.ImageWorkspaceCompletionCost,
		ImageWorkspaceCompletionCostMapJSON:                 settings.ImageWorkspaceCompletionCostMapJSON,
		ImageWorkspacePromptSafetyEnabled:                   settings.ImageWorkspacePromptSafetyEnabled,
		ImageWorkspaceAssumeWorkerReady:                     settings.ImageWorkspaceAssumeWorkerReady,
		ImageWorkspaceObjectStorageEnabled:                  settings.ImageWorkspaceObjectStorageEnabled,
		ImageWorkspaceObjectStorageProvider:                 settings.ImageWorkspaceObjectStorageProvider,
		ImageWorkspaceObjectStorageBucket:                   settings.ImageWorkspaceObjectStorageBucket,
		ImageWorkspaceObjectStorageRegion:                   settings.ImageWorkspaceObjectStorageRegion,
		ImageWorkspaceObjectStoragePrefix:                   settings.ImageWorkspaceObjectStoragePrefix,
		ImageWorkspaceObjectStoragePublicBaseURL:            settings.ImageWorkspaceObjectStoragePublicBaseURL,
		MediaCDNBaseURL:                                     settings.MediaCDNBaseURL,
		...
		SignupGrantRiskControlOAuthIdentityEnabled:          settings.SignupGrantRiskControlOAuthIdentityEnabled,
		SignupGrantRiskControlDeviceEnabled:                 settings.SignupGrantRiskControlDeviceEnabled,
		SignupGrantRiskControlDeviceLimit:                   settings.SignupGrantRiskControlDeviceLimit,
		SignupGrantRiskControlFreeDomainLimit:               settings.SignupGrantRiskControlFreeDomainLimit,
		SignupGrantRiskControlBlockedDomains:                settings.SignupGrantRiskControlBlockedDomains,
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	req.PasswordMinLength = service.NormalizePasswordMinLengthForSetting(req.PasswordMinLength)
	...
	if req.WeChatConnectEnabled {
		...
		req.WeChatConnectOpenAppID = strings.TrimSpace(firstNonEmpty(req.WeChatConnectOpenAppID, previousSettings.WeChatConnectOpenAppID))
		req.WeChatConnectMPAppID = strings.TrimSpace(firstNonEmpty(req.WeChatConnectMPAppID, previousSettings.WeChatConnectMPAppID))
		req.WeChatConnectMobileAppID = strings.TrimSpace(firstNonEmpty(req.WeChatConnectMobileAppID, previousSettings.WeChatConnectMobileAppID))

		if req.WeChatConnectOpenAppSecret == "" {
			req.WeChatConnectOpenAppSecret = strings.TrimSpace(previousSettings.WeChatConnectOpenAppSecret)
		}
		if req.WeChatConnectMPAppSecret == "" {
			req.WeChatConnectMPAppSecret = strings.TrimSpace(previousSettings.WeChatConnectMPAppSecret)
		}
		if req.WeChatConnectMobileAppSecret == "" {
			req.WeChatConnectMobileAppSecret = strings.TrimSpace(previousSettings.WeChatConnectMobileAppSecret)
		}
		...
		if req.WeChatConnectOpenEnabled || req.WeChatConnectMPEnabled {
			if req.WeChatConnectRedirectURL != "" && config.ValidateAbsoluteHTTPURL(req.WeChatConnectRedirectURL) != nil {
	...
	minCodexVersion := optionalTrimmedString(req.MinCodexVersion, previousSettings.MinCodexVersion)
	maxCodexVersion := optionalTrimmedString(req.MaxCodexVersion, previousSettings.MaxCodexVersion)
	codexCLIOnlyBlacklist := optionalTrimmedString(req.CodexCLIOnlyBlacklist, previousSettings.CodexCLIOnlyBlacklist)
	codexCLIOnlyWhitelist := optionalTrimmedString(req.CodexCLIOnlyWhitelist, previousSettings.CodexCLIOnlyWhitelist)
	codexCLIOnlyEngineFingerprintSignals := optionalTrimmedString(req.CodexCLIOnlyEngineFingerprintSignals, previousSettings.CodexCLIOnlyEngineFingerprintSignals)

	if minCodexVersion != "" && !semverPattern.MatchString(minCodexVersion) {
		response.Error(c, http.StatusBadRequest, "min_codex_version must be empty or a valid semver (e.g. 0.141.0)")
		return
	}
	if maxCodexVersion != "" && !semverPattern.MatchString(maxCodexVersion) {
	...
	if err := service.ValidateCodexClientEntriesJSON(codexCLIOnlyBlacklist); err != nil {
		response.Error(c, http.StatusBadRequest, "codex_cli_only_blacklist "+err.Error())
		return
	}
	if err := service.ValidateCodexWhitelistEntriesJSON(codexCLIOnlyWhitelist); err != nil {
		response.Error(c, http.StatusBadRequest, "codex_cli_only_whitelist "+err.Error())
		return
	}
	if err := service.ValidateEngineFingerprintSignalsJSON(codexCLIOnlyEngineFingerprintSignals); err != nil {
	...
	settings := &service.SystemSettings{
		...
		WeChatExportFetchRetries:                 optionalInt(req.WeChatExportFetchRetries, previousSettings.WeChatExportFetchRetries),
		WeChatExportFetchTimeoutMS:               optionalInt(req.WeChatExportFetchTimeoutMS, previousSettings.WeChatExportFetchTimeoutMS),
		WeChatExportWorkerConcurrency:            optionalInt(req.WeChatExportWorkerConcurrency, previousSettings.WeChatExportWorkerConcurrency),
		WeChatExportWorkerIntervalMS:             optionalInt(req.WeChatExportWorkerIntervalMS, previousSettings.WeChatExportWorkerIntervalMS),
		WeChatExportWorkerLeaseSeconds:           optionalInt(req.WeChatExportWorkerLeaseSeconds, previousSettings.WeChatExportWorkerLeaseSeconds),
		WeChatExportWorkerMaxBackoffMS:           optionalInt(req.WeChatExportWorkerMaxBackoffMS, previousSettings.WeChatExportWorkerMaxBackoffMS),
		ImageWorkspaceUpstreamURL:                optionalTrimmedString(req.ImageWorkspaceUpstreamURL, previousSettings.ImageWorkspaceUpstreamURL),
		ImageWorkspaceGenerationTimeoutMS:        optionalInt(req.ImageWorkspaceGenerationTimeoutMS, previousSettings.ImageWorkspaceGenerationTimeoutMS),
		ImageWorkspaceCompletionCost:             optionalTrimmedString(req.ImageWorkspaceCompletionCost, previousSettings.ImageWorkspaceCompletionCost),
		ImageWorkspaceCompletionCostMapJSON:      optionalTrimmedString(req.ImageWorkspaceCompletionCostMapJSON, previousSettings.ImageWorkspaceCompletionCostMapJSON),
		ImageWorkspacePromptSafetyEnabled:        optionalBool(req.ImageWorkspacePromptSafetyEnabled, previousSettings.ImageWorkspacePromptSafetyEnabled),
		ImageWorkspaceAssumeWorkerReady:          optionalBool(req.ImageWorkspaceAssumeWorkerReady, previousSettings.ImageWorkspaceAssumeWorkerReady),
		ImageWorkspaceObjectStorageEnabled:       optionalBool(req.ImageWorkspaceObjectStorageEnabled, previousSettings.ImageWorkspaceObjectStorageEnabled),
		ImageWorkspaceObjectStorageProvider:      optionalTrimmedString(req.ImageWorkspaceObjectStorageProvider, previousSettings.ImageWorkspaceObjectStorageProvider),
		ImageWorkspaceObjectStorageBucket:        optionalTrimmedString(req.ImageWorkspaceObjectStorageBucket, previousSettings.ImageWorkspaceObjectStorageBucket),
		ImageWorkspaceObjectStorageRegion:        optionalTrimmedString(req.ImageWorkspaceObjectStorageRegion, previousSettings.ImageWorkspaceObjectStorageRegion),
		ImageWorkspaceObjectStoragePrefix:        optionalTrimmedString(req.ImageWorkspaceObjectStoragePrefix, previousSettings.ImageWorkspaceObjectStoragePrefix),
		ImageWorkspaceObjectStoragePublicBaseURL: optionalTrimmedString(req.ImageWorkspaceObjectStoragePublicBaseURL, previousSettings.ImageWorkspaceObjectStoragePublicBaseURL),
		MediaCDNBaseURL:                          optionalTrimmedString(req.MediaCDNBaseURL, previousSettings.MediaCDNBaseURL),
		...
		CodexCLIOnlyBlacklist: codexCLIOnlyBlacklist,
		CodexCLIOnlyWhitelist: codexCLIOnlyWhitelist,
		...
		CodexCLIOnlyEngineFingerprintSignals: codexCLIOnlyEngineFingerprintSignals,
		...
		SignupGrantRiskControlOAuthIdentityEnabled: optionalBool(req.SignupGrantRiskControlOAuthIdentityEnabled, previousSettings.SignupGrantRiskControlOAuthIdentityEnabled),
		SignupGrantRiskControlDeviceEnabled:        optionalBool(req.SignupGrantRiskControlDeviceEnabled, previousSettings.SignupGrantRiskControlDeviceEnabled),
		SignupGrantRiskControlDeviceLimit:          optionalInt(req.SignupGrantRiskControlDeviceLimit, previousSettings.SignupGrantRiskControlDeviceLimit),
		SignupGrantRiskControlFreeDomainLimit:      optionalInt(req.SignupGrantRiskControlFreeDomainLimit, previousSettings.SignupGrantRiskControlFreeDomainLimit),
		SignupGrantRiskControlBlockedDomains:       optionalTrimmedString(req.SignupGrantRiskControlBlockedDomains, previousSettings.SignupGrantRiskControlBlockedDomains),
	...
	payload := dto.SystemSettings{
		...
		WeChatExportFetchRetries:                     updatedSettings.WeChatExportFetchRetries,
		WeChatExportFetchTimeoutMS:                   updatedSettings.WeChatExportFetchTimeoutMS,
		WeChatExportWorkerConcurrency:                updatedSettings.WeChatExportWorkerConcurrency,
		WeChatExportWorkerIntervalMS:                 updatedSettings.WeChatExportWorkerIntervalMS,
		WeChatExportWorkerLeaseSeconds:               updatedSettings.WeChatExportWorkerLeaseSeconds,
		WeChatExportWorkerMaxBackoffMS:               updatedSettings.WeChatExportWorkerMaxBackoffMS,
		ImageWorkspaceUpstreamURL:                    updatedSettings.ImageWorkspaceUpstreamURL,
		ImageWorkspaceGenerationTimeoutMS:            updatedSettings.ImageWorkspaceGenerationTimeoutMS,
		ImageWorkspaceCompletionCost:                 updatedSettings.ImageWorkspaceCompletionCost,
		ImageWorkspaceCompletionCostMapJSON:          updatedSettings.ImageWorkspaceCompletionCostMapJSON,
		ImageWorkspacePromptSafetyEnabled:            updatedSettings.ImageWorkspacePromptSafetyEnabled,
		ImageWorkspaceAssumeWorkerReady:              updatedSettings.ImageWorkspaceAssumeWorkerReady,
		ImageWorkspaceObjectStorageEnabled:           updatedSettings.ImageWorkspaceObjectStorageEnabled,
		ImageWorkspaceObjectStorageProvider:          updatedSettings.ImageWorkspaceObjectStorageProvider,
		ImageWorkspaceObjectStorageBucket:            updatedSettings.ImageWorkspaceObjectStorageBucket,
		ImageWorkspaceObjectStorageRegion:            updatedSettings.ImageWorkspaceObjectStorageRegion,
		ImageWorkspaceObjectStoragePrefix:            updatedSettings.ImageWorkspaceObjectStoragePrefix,
		ImageWorkspaceObjectStoragePublicBaseURL:     updatedSettings.ImageWorkspaceObjectStoragePublicBaseURL,
		MediaCDNBaseURL:                              updatedSettings.MediaCDNBaseURL,
		...
		SignupGrantRiskControlOAuthIdentityEnabled: updatedSettings.SignupGrantRiskControlOAuthIdentityEnabled,
		SignupGrantRiskControlDeviceEnabled:        updatedSettings.SignupGrantRiskControlDeviceEnabled,
		SignupGrantRiskControlDeviceLimit:          updatedSettings.SignupGrantRiskControlDeviceLimit,
		SignupGrantRiskControlFreeDomainLimit:      updatedSettings.SignupGrantRiskControlFreeDomainLimit,
		SignupGrantRiskControlBlockedDomains:       updatedSettings.SignupGrantRiskControlBlockedDomains,
...
func diffSettings(before *service.SystemSettings, after *service.SystemSettings, beforeAuthSourceDefaults *service.AuthSourceDefaultSettings, afterAuthSourceDefaults *service.AuthSourceDefaultSettings, req UpdateSettingsRequest) []string {
	...
	if before.PasswordMinLength != after.PasswordMinLength {
	...
	if before.SignupGrantRiskControlOAuthIdentityEnabled != after.SignupGrantRiskControlOAuthIdentityEnabled {
		changed = append(changed, service.SettingKeySignupGrantRiskControlOAuthIdentityEnabled)
	}
	if before.SignupGrantRiskControlDeviceEnabled != after.SignupGrantRiskControlDeviceEnabled {
		changed = append(changed, service.SettingKeySignupGrantRiskControlDeviceEnabled)
	}
	if before.SignupGrantRiskControlDeviceLimit != after.SignupGrantRiskControlDeviceLimit {
		changed = append(changed, service.SettingKeySignupGrantRiskControlDeviceLimit)
	}
	if before.SignupGrantRiskControlFreeDomainLimit != after.SignupGrantRiskControlFreeDomainLimit {
		changed = append(changed, service.SettingKeySignupGrantRiskControlFreeDomainLimit)
	}
	if before.SignupGrantRiskControlBlockedDomains != after.SignupGrantRiskControlBlockedDomains {
		changed = append(changed, service.SettingKeySignupGrantRiskControlBlockedDomains)
	}
	if before.SignupGrantRiskControlFreeDomains != after.SignupGrantRiskControlFreeDomains {
		changed = append(changed, service.SettingKeySignupGrantRiskControlFreeDomains)
	}
	if before.SignupGrantRiskControlTrustedDomains != after.SignupGrantRiskControlTrustedDomains {
		changed = append(changed, service.SettingKeySignupGrantRiskControlTrustedDomains)
	}
