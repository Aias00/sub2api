...
func TestSettingHandler_UpdateSettings_AllowsBlankWeChatPaymentOAuthRedirectURL(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &settingHandlerRepoStub{
		values: map[string]string{},
	}
	...
	body := map[string]any{
		...
		"wechat_connect_mp_app_id":     "wx-mp-app",
		"wechat_connect_mp_app_secret": "wx-mp-secret",
	...
	require.Equal(t, http.StatusOK, rec.Code)
	require.Equal(t, "true", repo.values[service.SettingKeyWeChatConnectEnabled])
	require.Equal(t, "true", repo.values[service.SettingKeyWeChatConnectMPEnabled])
	require.Equal(t, "", repo.values[service.SettingKeyWeChatConnectRedirectURL])
}
