...
func TestNormalizeRegistrationEmailSuffixWhitelistInvalid(t *testing.T) {
	for _, item := range []string{"@", "@invalid_domain", "foo", "*.foo", "bad@foo.com"} {
...
func TestRegistrationEmailSuffix(t *testing.T) {
	require.Equal(t, "@example.com", RegistrationEmailSuffix("User@Example.COM"))
	require.Empty(t, RegistrationEmailSuffix("bad"))
}
