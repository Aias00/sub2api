...
func NewPromptCatalogRepository(db *sql.DB) PromptCatalogRepository {
...
func (r *promptCatalogRepository) ListCases(
	...
	filters PromptCatalogListFilters,
) ([]PromptCatalogCase, *pagination.PaginationResult, error) {
...
LIMIT $` + fmt.Sprint(len(queryArgs)-1) + ` OFFSET $` + fmt.Sprint(len(queryArgs))
	...
	items := make([]PromptCatalogCase, 0, params.Limit())
	for rows.Next() {
...
func (r *promptCatalogRepository) GetCaseSummary(ctx context.Context, filters PromptCatalogListFilters) (*PromptCatalogSummary, error) {
	whereSQL, args := buildPromptCatalogWhere(filters)
...
func (r *promptCatalogRepository) GetCaseByID(ctx context.Context, id string) (*PromptCatalogCase, error) {
	query := promptCatalogSelectSQL() + `
WHERE id = $1 AND status = $2 AND deleted_at IS NULL
LIMIT 1`
	item, err := scanPromptCatalogCase(r.db.QueryRowContext(ctx, query, id, PromptCatalogStatusPublished))
	if err != nil {
...
func (r *promptCatalogRepository) UpsertCase(ctx context.Context, item *PromptCatalogCase) error {
...
func buildPromptCatalogWhere(filters PromptCatalogListFilters) (string, []any) {
	clauses := []string{"status = $1", "deleted_at IS NULL"}
	args := []any{PromptCatalogStatusPublished}
...
func scanPromptCatalogCase(row promptCatalogScanner) (PromptCatalogCase, error) {
	var item PromptCatalogCase
...
func promptCatalogFacetCounts(counts map[string]int64, labels map[string]string, countFirst bool) []PromptCatalogFacetCount {
	facets := make([]PromptCatalogFacetCount, 0, len(counts))
	for value, count := range counts {
		facets = append(facets, PromptCatalogFacetCount{Value: value, Label: labels[value], Count: count})
	}
