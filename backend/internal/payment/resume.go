...
func NewResumeService(signingKey []byte, verifyFallbacks ...[]byte) *ResumeService {
	svc := &ResumeService{}
	if len(signingKey) > 0 {
...
func (s *ResumeService) IsSigningConfigured() bool {
	return s != nil && len(s.signingKey) > 0
}

func (s *ResumeService) EnsureSigningKey() error {
...
func (s *ResumeService) CreateToken(claims ResumeTokenClaims) (string, error) {
...
func (s *ResumeService) ParseToken(token string) (*ResumeTokenClaims, error) {
	if err := s.EnsureSigningKey(); err != nil {
		return nil, err
	}
...
func (s *ResumeService) CreateWeChatPaymentResumeToken(claims WeChatPaymentResumeClaims) (string, error) {
	if err := s.EnsureSigningKey(); err != nil {
		return "", err
	}
...
func (s *ResumeService) ParseWeChatPaymentResumeToken(token string) (*WeChatPaymentResumeClaims, error) {
	if err := s.EnsureSigningKey(); err != nil {
		return nil, err
	}
...
func (s *ResumeService) createSignedToken(claims any) (string, error) {
...
func (s *ResumeService) parseSignedToken(token string, dest any) error {
...
func (s *ResumeService) verifySignature(payload string, signature string) bool {
	if s == nil {
		return false
	}
...
func (s *ResumeService) sign(payload string) string {
