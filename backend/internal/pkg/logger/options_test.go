...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/cloudbase-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/cloudbase-data", "logs", "cloudbase.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "cloudbase.log")
