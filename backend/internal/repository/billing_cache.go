...
const (
	...
	// Rate limit window durations must match gateway API key rate-limit windows.
...
func NewBillingCache(rdb *redis.Client) *billingCache {
...
func (c *billingCache) GetSubscriptionCache(ctx context.Context, userID, groupID int64) (*billingctx.SubscriptionCacheData, error) {
...
func (c *billingCache) parseSubscriptionCache(data map[string]string) (*billingctx.SubscriptionCacheData, error) {
	result := &billingctx.SubscriptionCacheData{}
...
func (c *billingCache) SetSubscriptionCache(ctx context.Context, userID, groupID int64, data *billingctx.SubscriptionCacheData) error {
	if data == nil {
		return nil
	}
...
func (c *billingCache) GetAPIKeyRateLimit(ctx context.Context, keyID int64) (*billingctx.APIKeyRateLimitCacheData, error) {
	...
	data := &billingctx.APIKeyRateLimitCacheData{}
...
func (c *billingCache) SetAPIKeyRateLimit(ctx context.Context, keyID int64, data *billingctx.APIKeyRateLimitCacheData) error {
	if data == nil {
		return nil
	}
	key := billingRateLimitKey(keyID)
...
// *billingctx.UserPlatformQuotaCacheEntry。空 map（key 不存在）返回 nil。
// GetUserPlatformQuotaCache 和 BatchGetUserPlatformQuotaCache 共用此函数，确保解析逻辑一致。
func parseUserPlatformQuotaHash(m map[string]string) *billingctx.UserPlatformQuotaCacheEntry {
	if len(m) == 0 {
		return nil
	}
	...
	return &billingctx.UserPlatformQuotaCacheEntry{
...
func (c *billingCache) GetUserPlatformQuotaCache(ctx context.Context, userID int64, platform string) (*billingctx.UserPlatformQuotaCacheEntry, bool, error) {
...
func (c *billingCache) SetUserPlatformQuotaCache(ctx context.Context, userID int64, platform string, entry *billingctx.UserPlatformQuotaCacheEntry, ttl time.Duration) error {
	if entry == nil {
		return nil
	}
	key := userPlatformQuotaCacheKey(userID, platform)
...
func (c *billingCache) IncrUserPlatformQuotaUsageCache(ctx context.Context, userID int64, platform string, cost float64, ttl time.Duration, markDirty bool) error {
	...
	_, err := c.rdb.Eval(ctx, updateUserPlatformQuotaUsageScript,
		...
		int(ttl.Seconds()),
		billingctx.UserPlatformQuotaCacheSchemaV1,
		member,
...
// billingctx.UserPlatformQuotaKey。解析失败返回 ok=false。
func parseUserPlatformQuotaDirtyMember(m string) (billingctx.UserPlatformQuotaKey, bool) {
	parts := strings.SplitN(m, ":", 2)
	if len(parts) != 2 {
		return billingctx.UserPlatformQuotaKey{}, false
	}
	uid, err := strconv.ParseInt(parts[0], 10, 64)
	if err != nil {
		return billingctx.UserPlatformQuotaKey{}, false
	}
	return billingctx.UserPlatformQuotaKey{UserID: uid, Platform: parts[1]}, true
}

// PopDirtyUserPlatformQuotaKeys 从脏集随机弹出最多 n 个 key。
// 脏集为空时返回 (nil, nil)。
func (c *billingCache) PopDirtyUserPlatformQuotaKeys(ctx context.Context, n int) ([]billingctx.UserPlatformQuotaKey, error) {
	...
	keys := make([]billingctx.UserPlatformQuotaKey, 0, len(members))
...
// 空切片时直接返回 nil。
func (c *billingCache) ReaddDirtyUserPlatformQuotaKeys(ctx context.Context, keys []billingctx.UserPlatformQuotaKey) error {
	if len(keys) == 0 {
		return nil
	}
...
func (c *billingCache) BatchGetUserPlatformQuotaCache(ctx context.Context, keys []billingctx.UserPlatformQuotaKey) ([]*billingctx.UserPlatformQuotaCacheEntry, error) {
	if len(keys) == 0 {
		return nil, nil
	}
	pipe := c.rdb.Pipeline()
	...
	results := make([]*billingctx.UserPlatformQuotaCacheEntry, len(keys))
