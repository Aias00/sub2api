...
func (s *ConcurrencyCacheSuite) SetupTest() {
	...
	s.rawCache = NewConcurrencyCache(s.rdb, testSlotTTLMinutes, int(testSlotTTL.Seconds()))
	s.cache = s.rawCache
}

func (s *ConcurrencyCacheSuite) redisUnixSeconds() int64 {
...
func (s *ConcurrencyCacheSuite) TestAPIKeySlot_TrackReleaseAndBatchCount() {
	svc := service.NewConcurrencyService(s.cache)
	apiKeyID := int64(300)
	emptyAPIKeyID := int64(301)

	release := svc.TrackAPIKeySlot(s.ctx, apiKeyID)
	release()
	counts, err := svc.GetAPIKeyConcurrencyBatch(s.ctx, []int64{apiKeyID, emptyAPIKeyID})
	require.NoError(s.T(), err)
	require.Equal(s.T(), map[int64]int{apiKeyID: 0, emptyAPIKeyID: 0}, counts)
}
