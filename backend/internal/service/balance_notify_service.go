...
func (s *BalanceNotifyService) buildBalanceLowEmailBodyWithLogo(userName string, balance, threshold float64, siteName, rechargeURL, logoURL string) string {
	...
	support := emailDetailsBlock("", []emailDetailRow{
		...
		{Label: "Alert threshold", Value: fmt.Sprintf("$%.2f", threshold)},
	})
	if rechargeURL != "" {
		support += emailActionButton("立即充值 / Top Up Now", rechargeURL)
	}
...
func (s *BalanceNotifyService) buildQuotaAlertEmailBodyWithLogo(accountID int64, accountName, platform, dimLabel string, used, limit, remaining float64, thresholdDisplay, siteName, logoURL string) string {
	...
	return renderProductEmail(emailTemplateData{
		...
		SupportHTML: emailDetailsBlock("账号限额告警 / Account Quota Alert", []emailDetailRow{
			{Label: "账号 ID / Account ID", Value: fmt.Sprintf("#%d", accountID)},
			{Label: "账号 / Account", Value: accountName},
			{Label: "平台 / Platform", Value: platform},
			{Label: "维度 / Dimension", Value: dimLabel},
			{Label: "已使用 / Used", Value: fmt.Sprintf("$%.2f", used)},
			{Label: "限额 / Limit", Value: limitStr},
			{Label: "剩余额度 / Remaining", Value: fmt.Sprintf("$%.2f", remaining)},
			{Label: "提醒阈值 / Alert Threshold", Value: thresholdDisplay},
		}),
		FooterHTML: emailMutedParagraph("账号剩余额度已低于提醒阈值，请及时关注。") +
			emailMutedParagraph("Account remaining quota has fallen below the alert threshold."),
	})
}
