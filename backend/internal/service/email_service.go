...
func (s *EmailService) buildVerifyCodeEmailBodyWithLogo(code, siteName, logoURL string) string {
	...
	return renderProductEmail(emailTemplateData{
		...
		FooterHTML: emailMutedParagraph("This code expires in 15 minutes.") +
			emailMutedParagraph(fmt.Sprintf("If you didn't request this code for %s, you can safely ignore this email. Someone else might have typed your email address by mistake.", siteName)),
	})
}
...
func (s *EmailService) buildPasswordResetEmailBodyWithLogo(resetURL, siteName, logoURL string) string {
	...
	return renderProductEmail(emailTemplateData{
		...
		SupportHTML: emailFallbackLink("If the button doesn't work, copy and paste this link into your browser:", resetURL),
		FooterHTML: emailMutedParagraph("This link expires in 30 minutes.") +
			emailMutedParagraph("If you didn't request a password reset, you can safely ignore this email. Your password will stay unchanged."),
	})
}
