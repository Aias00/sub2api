...
func TestOpenAIImageOutputCounter_NestedCloudbase_StandardResponse(t *testing.T) {
	// Simulate what a nested cloudbase setup receives
	// When the upstream cloudbase returns a standard response, the downstream
	// cloudbase receives SSE events. This test verifies that no false positives
	// occur.
...
data: [DONE]`
	...
	count := countOpenAIImageOutputsFromSSEBody(sseBody)
	if count != 0 {
		t.Errorf("expected 0 images for nested cloudbase text-only response, got %d", count)
	}
}
