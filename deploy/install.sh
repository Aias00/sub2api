...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="Cloudbase 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 Cloudbase 服务："
    ...
    ["not_installed"]="Cloudbase 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 Cloudbase。"
    ...
    ["uninstall_complete"]="Cloudbase 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 Cloudbase"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 Cloudbase"
    ...
    ["server_config_desc"]="配置 Cloudbase 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="Cloudbase Installation Script"
    ...
    # Completion
    ["install_complete"]="Cloudbase installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start Cloudbase service:"
    ...
    ["not_installed"]="Cloudbase is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove Cloudbase from your system."
    ...
    ["uninstall_complete"]="Cloudbase has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install Cloudbase"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove Cloudbase"
    ...
    ["server_config_desc"]="Configure Cloudbase server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/cloudbase" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/cloudbase" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="cloudbase_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/cloudbase" "$INSTALL_DIR/cloudbase"
    chmod +x "$INSTALL_DIR/cloudbase"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/cloudbase"
}
...
User=cloudbase
Group=cloudbase
WorkingDirectory=/opt/cloudbase
ExecStart=/opt/cloudbase/cloudbase
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=cloudbase
...
start_service() {
    ...
    if systemctl start cloudbase; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u cloudbase -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable cloudbase 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status cloudbase"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u cloudbase -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart cloudbase"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop cloudbase"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if Cloudbase is installed
    if [ ! -f "$INSTALL_DIR/cloudbase" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/cloudbase" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet cloudbase; then
        print_info "$(msg 'stopping_service')"
        systemctl stop cloudbase
    fi

    # Backup current binary
    cp "$INSTALL_DIR/cloudbase" "$INSTALL_DIR/cloudbase.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/cloudbase.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/cloudbase"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start cloudbase

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: Cloudbase must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/cloudbase" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="cloudbase.backup.${current_version}"
        else
            backup_name="cloudbase.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/cloudbase" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop cloudbase 2>/dev/null || true
    systemctl disable cloudbase 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/cloudbase.service
    systemctl daemon-reload
