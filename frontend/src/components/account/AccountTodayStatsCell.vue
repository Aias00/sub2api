<template>
  <div>
    ...
    <div v-else-if="props.stats" class="space-y-0.5 text-xs">
      ...
      <!-- Cost (Account) -->
      <div class="flex items-center gap-1">
        <span class="text-gray-500 dark:text-gray-400">{{ t('common.billingMetrics.accountBilled') }}:</span>
      ...
      <div v-if="props.stats.user_cost != null" class="flex items-center gap-1">
        <span class="text-gray-500 dark:text-gray-400">{{ t('common.billingMetrics.userBilled') }}:</span>
        <span class="font-medium text-gray-700 dark:text-gray-300">{{
