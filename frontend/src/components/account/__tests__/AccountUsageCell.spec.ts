...
describe('AccountUsageCell', () => {
  ...
  it('OpenAI OAuth 在行数据刷新但仍无 codex 快照时不会隐式重拉 usage', async () => {
	getUsage
  ...
  it('Key 账号会展示 today stats 徽章并带 A/U 提示', async () => {
		...
		expect(badges.some(node => node.attributes('title') === 'common.billingMetrics.accountBilled')).toBe(true)
		expect(badges.some(node => node.attributes('title') === 'common.billingMetrics.userBilled')).toBe(true)
  })
