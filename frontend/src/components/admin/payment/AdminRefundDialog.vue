    ...
    <form id="refund-form" @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div class="rounded-lg bg-gray-50 p-3 dark:bg-dark-700">
        ...
        <div class="mt-1 flex justify-between text-sm">
          ...
          <span class="font-medium text-gray-900 dark:text-white">{{ order ? formatOrderCreditedAmount(order) : '0.00' }}</span>
        </div>
        <div class="mt-1 flex justify-between text-sm">
          <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</span>
          <span class="font-medium text-gray-900 dark:text-white">{{ order ? formatOrderPayAmount(order) : '0.00' }}</span>
        </div>
        <div v-if="actuallyRefunded > 0" class="mt-1 flex justify-between text-sm">
          ...
          <span class="font-medium text-red-600 dark:text-red-400">{{ order ? formatOrderCreditedAmount(order, actuallyRefunded) : '0.00' }}</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div v-if="form.deduct_balance && userBalance != null" class="mt-3 grid grid-cols-2 gap-3">
          ...
          <div class="rounded-lg bg-gray-50 p-3 text-sm dark:bg-dark-700">
            ...
            <div class="mt-1 font-semibold text-gray-900 dark:text-white">{{ order ? formatOrderCreditedAmount(order) : '0.00' }}</div>
          </div>
        </div>
      ...
      <div>
        ...
        <div class="relative">
          ...
          <input
            ...
            min="0.01"
            :max="maxRefundable"
            :class="['input', refundAmountPrefix ? 'pl-7' : '']"
            required
          />
        </div>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          {{ t('payment.admin.maxRefundable') }}: {{ order ? formatOrderCreditedAmount(order, maxRefundable) : '0.00' }}
        </p>
      </div>
...
import { formatOrderCreditedAmount, formatOrderPayAmount, resolvePaymentCurrencyPrefix } from '@/utils/paymentCurrency'
...
const refundAmountPrefix = computed(() => {
  if (!props.order || props.order.order_type === 'balance') return ''
  return resolvePaymentCurrencyPrefix(props.order.currency)
})
