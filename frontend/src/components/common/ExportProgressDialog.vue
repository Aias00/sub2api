<template>
  <BaseDialog :show="show" :title="t('common.exportProgress.exporting')" width="narrow" @close="handleCancel">
    <div class="space-y-4">
      <div class="text-sm text-gray-600 dark:text-gray-400">
        {{ t('common.exportProgress.exportingProgress') }}
      </div>
      <div class="flex items-center justify-between text-sm text-gray-700 dark:text-gray-300">
        <span>{{ t('common.exportProgress.exportedCount', { current, total }) }}</span>
      ...
      <div class="h-2 w-full rounded-full bg-gray-200 dark:bg-dark-700">
        <div
          ...
          aria-valuemin="0"
          aria-valuemax="100"
          :aria-label="`${t('common.exportProgress.exportingProgress')}: ${normalizedProgress}%`"
      ...
      <div v-if="estimatedTime" class="text-xs text-gray-500 dark:text-gray-400" aria-live="polite" aria-atomic="true">
        {{ t('common.exportProgress.estimatedTime', { time: estimatedTime }) }}
      </div>
    </div>
    ...
    <template #footer>
      ...
      >
        {{ t('common.exportProgress.cancelExport') }}
      </button>
    </template>
  </BaseDialog>
</template>
