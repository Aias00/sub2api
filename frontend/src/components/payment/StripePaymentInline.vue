<template>
  <div class="space-y-4">
    ...
    <div v-else-if="initError" class="card p-6 text-center">
      ...
      <button class="btn btn-secondary mt-4" @click="$emit('back')">{{ paymentText('backToRecharge') }}</button>
    </div>
    <!-- Success -->
    <template v-else-if="success">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <button class="btn btn-primary" @click="$emit('done')">{{ paymentText('confirm') }}</button>
        </div>
      </div>
    </template>
    <template v-else>
...
const gatewayCurrency = computed(() => normalizePaymentCurrency(props.currency))
const localeCode = computed(() => {
  const raw = locale.value
...
function paymentText(key: StripeInlineLabelKey): string {
  return renderStripeInlineText(props.labels, key)
}

function paymentResultReturnURL(): string {
  const target = new URL(authRouteDefaults.value.paymentResultPath, window.location.origin)
  target.searchParams.set('order_id', String(props.orderId))
...
async function handlePay() {
  ...
  if (POPUP_METHODS.has(selectedType.value)) {
    const popupUrl = router.resolve({
      ...
      query: {
        ...
        currency: gatewayCurrency.value,
      },
    }).href
  ...
  try {
    ...
    if (stripeError) {
      error.value = stripeError.message || paymentText('failed')
    } else {
      success.value = true
      emit('success')
    }
  } catch (err: unknown) {
    error.value = extractI18nErrorMessage(err, t, 'payment.errors', paymentText('failed'))
  } finally {
