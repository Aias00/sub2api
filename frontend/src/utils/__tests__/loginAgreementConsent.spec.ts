...
describe('loginAgreementConsent', () => {
  ...
  it('ignores malformed or incomplete session state', () => {
    sessionStorage.setItem('cloudbase_login_agreement_attempt', JSON.stringify({
      revision: 'legacy-rev',
      accepted_at: '2026-05-22T00:00:00.000Z',
    }))

    expect(hasAcceptedLoginAgreement('legacy-rev')).toBe(true)
    sessionStorage.setItem('cloudbase_login_agreement_attempt', JSON.stringify({ accepted_at: '2026-05-22T00:00:00.000Z' }))
