      ...
      <div v-else class="space-y-6">
                  ...
                  <div>
                    ...
                    <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                      <span
                        ...
                        :class="form.invitation_code_enabled ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-gray-100 text-gray-700 dark:bg-dark-700 dark:text-dark-200'"
                      >
                        {{ form.invitation_code_enabled ? t('admin.affiliates.moduleOverview.enabled') : t('admin.affiliates.moduleOverview.disabled') }}
                      </span>
                    </p>
                  </div>
                  <router-link
                    to="/admin/affiliates/rules"
                    class="btn btn-secondary btn-sm"
                  >
                    {{ t('nav.affiliateRules') }}
                  </router-link>
                </div>
              </div>
        ...
        <div class="card">
          ...
          <div class="space-y-5 p-6">
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-900">
                ...
                <p class="mt-2 text-lg font-semibold text-gray-900 dark:text-white">
                  {{ form.affiliate_enabled ? t('admin.affiliates.moduleOverview.enabled') : t('admin.affiliates.moduleOverview.disabled') }}
                </p>
              </div>
              <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-900">
                <p class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-dark-400">
                ...
                <p class="mt-2 text-lg font-semibold text-gray-900 dark:text-white">
                  {{ Number(form.affiliate_rebate_rate || 0).toFixed(2) }}%
                </p>
              </div>
              <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-900">
                <p class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-dark-400">
                ...
                <p class="mt-2 text-lg font-semibold text-gray-900 dark:text-white">
                  {{
                    Number(form.affiliate_rebate_freeze_hours || 0) > 0
                      ? t('admin.affiliates.moduleOverview.hoursUnit', { count: Number(form.affiliate_rebate_freeze_hours || 0) })
                      : t('admin.affiliates.moduleOverview.disabled')
                  }}
                </p>
              </div>
              <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-900">
                ...
                <p class="mt-2 text-lg font-semibold text-gray-900 dark:text-white">
                  {{
                    Number(form.affiliate_rebate_duration_days || 0) > 0
                      ? t('admin.affiliates.moduleOverview.daysUnit', { count: Number(form.affiliate_rebate_duration_days || 0) })
                      : t('admin.affiliates.moduleOverview.noLimit')
                  }}
                </p>
              </div>
              <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-900">
                ...
                <p class="mt-2 text-lg font-semibold text-gray-900 dark:text-white">
                  {{
                    Number(form.affiliate_rebate_per_invitee_cap || 0) > 0
                      ? Number(form.affiliate_rebate_per_invitee_cap || 0).toFixed(2)
                      : t('admin.affiliates.moduleOverview.noLimit')
                  }}
                </p>
              </div>
            ...
            <div class="rounded-xl border border-dashed border-primary-300 bg-primary-50/60 p-4 dark:border-primary-800 dark:bg-primary-900/10">
              <p class="text-sm text-primary-800 dark:text-primary-200">
                {{ t('admin.affiliates.rulesDescription') }}
              </p>
              <div class="mt-4 flex flex-wrap gap-3">
                <router-link
                  to="/admin/affiliates/overview"
                  class="btn btn-secondary btn-sm"
                >
                  {{ t('nav.affiliateOverview') }}
                </router-link>
                <router-link
                  to="/admin/affiliates/rules"
                  class="btn btn-secondary btn-sm"
                ...
                <router-link
                  to="/admin/affiliates/codes"
                  class="btn btn-secondary btn-sm"
                >
                  {{ t('nav.affiliateCodeManagement') }}
                </router-link>
                <router-link
                  to="/admin/affiliates/rebates"
                  class="btn btn-secondary btn-sm"
                >
                  {{ t('nav.affiliateRebateRecords') }}
                </router-link>
              </div>
            </div>
          </div>
          ...
          <div class="card">
            ...
            <div class="space-y-4 p-6">
              ...
              <template v-if="form.payment_enabled">
                ...
                <!-- Row 4: Enabled payment types (provider badges for external billing labels) -->
                <div>
                  <label class="input-label">{{
...
const paymentGuideHref = computed(() =>
  resolveRuntimeLanguage(locale) === "zh"
    ? "https://github.com/Aias00/cloudbase/blob/main/docs/PAYMENT_CN.md"
    : "https://github.com/Aias00/cloudbase/blob/main/docs/PAYMENT.md",
);

const paymentMethodsHref = computed(() =>
  resolveRuntimeLanguage(locale) === "zh"
    ? "https://github.com/Aias00/cloudbase/blob/main/docs/PAYMENT_CN.md#支持的支付方式"
    : "https://github.com/Aias00/cloudbase/blob/main/docs/PAYMENT.md#supported-payment-methods",
);
...
const allPaymentTypes = computed(() => [
  ...
  { value: "waffo", label: t("payment.methods.waffo") },
...
const providerKeyOptions = computed(() => [
  ...
  { value: "waffo", label: t("admin.settings.payment.providerWaffo") },
