<template>
  <AppLayout>
    <div class="space-y-4">
      ...
      <!-- Table -->
      <OrderTable :orders="orders" :loading="ordersLoading" show-user :labels="orderTableLabels">
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            ...
            <template v-if="row.status === 'REFUND_REQUESTED'">
              <span v-if="row.refund_amount" class="rounded-full bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">{{ formatOrderCreditedAmount(row, row.refund_amount) }}</span>
    ...
    <BaseDialog :show="showDetailDialog" :title="t('payment.admin.orderDetail')" width="wide" @close="showDetailDialog = false">
      <div v-if="selectedOrder" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.status') }}</p><OrderStatusBadge :status="selectedOrder.status" :labels="statusBadgeLabels" /></div>
          ...
          <div v-if="selectedOrder.refund_amount"><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.refundAmount') }}</p><p class="text-sm font-medium text-red-600 dark:text-red-400">{{ formatOrderCreditedAmount(selectedOrder, selectedOrder.refund_amount) }}</p></div>
...
import { formatOrderCreditedAmount, formatOrderPayAmount } from '@/utils/paymentCurrency'
...
const orderTableLabels = computed(() => ({
  ...
  fee: t('payment.orders.fee'),
  creditedAmount: t('payment.orders.creditedAmount'),
  user: t('payment.admin.colUser'),
  methodAlipay: t('payment.methods.alipay'),
  methodWxpay: t('payment.methods.wxpay'),
  methodStripe: t('payment.methods.stripe'),
  methodAirwallex: t('payment.methods.airwallex'),
  statusPending: t('payment.status.pending'),
  statusPaid: t('payment.status.paid'),
  statusRecharging: t('payment.status.recharging'),
  statusCompleted: t('payment.status.completed'),
  statusExpired: t('payment.status.expired'),
  statusCancelled: t('payment.status.cancelled'),
  statusFailed: t('payment.status.failed'),
  statusRefundRequested: t('payment.status.refund_requested'),
  statusRefunding: t('payment.status.refunding'),
  statusRefunded: t('payment.status.refunded'),
  statusPartiallyRefunded: t('payment.status.partially_refunded'),
  statusRefundFailed: t('payment.status.refund_failed'),
}))

const statusBadgeLabels = computed(() => ({
  PENDING: t('payment.status.pending'),
  PAID: t('payment.status.paid'),
  RECHARGING: t('payment.status.recharging'),
  ...
  REFUND_REQUESTED: t('payment.status.refund_requested'),
  REFUNDING: t('payment.status.refunding'),
