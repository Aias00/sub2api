  ...
  <BaseDialog :show="show" :title="plan ? t('payment.admin.editPlan') : t('payment.admin.createPlan')" width="wide" @close="emit('close')">
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <div>
        ...
        <input v-model="planForm.creem_product_id" type="text" class="input" :placeholder="t('payment.admin.creemProductIdPlaceholder')" />
      </div>
      <div class="grid grid-cols-2 gap-4">
...
const planForm = reactive({ name: '', group_id: null as number | null, description: '', price: 0, original_price: 0, creem_product_id: '', validity_days: 30, validity_unit: 'days', sort_order: 0, for_sale: true })
...
watch(() => props.show, (visible) => {
  if (!visible) return
  if (props.plan) {
    Object.assign(planForm, { name: props.plan.name, group_id: props.plan.group_id, description: props.plan.description, price: props.plan.price, original_price: props.plan.original_price || 0, creem_product_id: props.plan.creem_product_id || '', validity_days: props.plan.validity_days, validity_unit: props.plan.validity_unit || '', sort_order: props.plan.sort_order || 0, for_sale: props.plan.for_sale })
    planFeaturesText.value = (props.plan.features || []).join('\n')
  } else {
    Object.assign(planForm, { name: '', group_id: null, description: '', price: 0, original_price: 0, creem_product_id: '', validity_days: 30, validity_unit: 'days', sort_order: 0, for_sale: true })
...
function buildPlanPayload() {
  ...
  return {
    ...
    creem_product_id: planForm.creem_product_id.trim(),
