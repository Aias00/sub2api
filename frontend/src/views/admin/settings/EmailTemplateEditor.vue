        ...
        >
          <div class="flex flex-wrap items-center gap-2">
            ...
            <span
              ...
              "
            >
              {{ selectedEventMeta.optional ? t('admin.settings.emailTemplates.badges.optional') : t('admin.settings.emailTemplates.badges.transactional') }}
            </span>
          </div>
...
const eventMetaKeyMap: Record<string, string> = {
  "auth.verify_code": "authVerifyCode",
  "auth.password_reset": "authPasswordReset",
  "notification_email.verify_code": "notificationEmailVerifyCode",
  "subscription.purchase_success": "subscriptionPurchaseSuccess",
  "subscription.expiry_reminder": "subscriptionExpiryReminder",
  "balance.low": "balanceLow",
  "balance.recharge_success": "balanceRechargeSuccess",
  "account.quota_alert": "accountQuotaAlert",
  "content_moderation.violation_notice": "contentModerationViolationNotice",
  "content_moderation.account_disabled": "contentModerationAccountDisabled",
  "ops.alert": "opsAlert",
  "ops.scheduled_report": "opsScheduledReport",
}
...
function eventMetaFor(option?: EmailTemplateOption | null) {
  ...
  const metaKey = eventMetaKeyMap[option.value]
  const label = metaKey
    ? t(`admin.settings.emailTemplates.events.${metaKey}.label`)
    : (option.label || option.value)
  const timing = metaKey
    ? t(`admin.settings.emailTemplates.events.${metaKey}.timing`)
    : (option.description || "")
  const categoryLabel = metaKey
    ? t(`admin.settings.emailTemplates.events.${metaKey}.category`)
...
function formatCategory(category: string): string {
  ...
  return key ? t(`admin.settings.emailTemplates.categories.${key}`) : category;
}
