...
const { t, te } = useI18n()
const { authText, authRouteDefaults, defaultBindRedirectPath, defaultRedirectPath, loadAuthShellConfig } = useAuthShellText()
...
async function finalizePendingAccountResponse(completion: DingTalkPendingActionResponse) {
  ...
  if (completion.step === 'email_completion' || (completion as Record<string, unknown>)['requires_email_completion'] === true) {
    await router.replace({ path: authRouteDefaults.value.dingtalkEmailCompletionPath, query: { redirect } })
    return
  }
...
onMounted(async () => {
  ...
  try {
    ...
    if (completionData.step === 'email_completion' || (completionData as unknown as Record<string, unknown>)['requires_email_completion'] === true) {
      ...
      await router.replace({ path: authRouteDefaults.value.dingtalkEmailCompletionPath, query: { redirect: completionRedirect } })
      return
    }
