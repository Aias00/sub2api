<template>
  <div class="vercel-auth-shell flex min-h-screen items-center justify-center bg-gray-50 px-4 dark:bg-dark-900">
...
async function resolveOrderFromResumeToken(resumeToken: string): Promise<PaymentOrder | null> {
  try {
    ...
    return publicVerifyResultToOrder(result.data)
  } catch (_err: unknown) {
    return null
  }
}
