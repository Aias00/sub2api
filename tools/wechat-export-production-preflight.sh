...
if [[ ! -f tools/wechat-worker/Dockerfile ]]; then
  ...
  if grep -q "HEALTHCHECK" tools/wechat-worker/Dockerfile \
    && grep -q -- "--healthcheck" tools/wechat-worker/Dockerfile; then
...
if command -v docker >/dev/null 2>&1; then
  if POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-dummy}" docker compose -f deploy/docker-compose.yml -f deploy/docker-compose.wechat-worker.yml --profile wechat-worker config >/tmp/sub2api-wechat-worker-compose.yml; then
