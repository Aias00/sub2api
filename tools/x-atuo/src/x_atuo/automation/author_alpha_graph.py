...
class AuthorAlphaExecutionGraph:
    ...
    async def invoke(self, request: AutomationRequest) -> WorkflowStateModel:
        ...
        for index, candidate in enumerate(post_queue):
            ...
            if index < len(post_queue) - 1 and len(consumed_target_tweet_ids) < max_targets:
                await maybe_await(self.sleep(int(self.config.author_alpha.target_switch_delay_seconds)))
    ...
    def _excluded_author_set(self) -> set[str]:
        return {
            str(author).strip()
    ...
    def _ordered_authors(self) -> list[dict[str, Any]]:
        excluded = self._excluded_author_set()
        authors = [
            author
    ...
    async def _execute_candidate_burst(
        ...
        for burst_index in range(1, max_burst + 1):
            ...
            if self._should_pause_before_next_reply(candidate, burst_index=burst_index):
                await maybe_await(self.sleep(int(self.config.author_alpha.inter_reply_delay_seconds)))
    ...
    def _candidate_current_day_reason(self, candidate: FeedCandidate) -> str | None:
        return candidate_current_day_reason(candidate, now=self._now())
    ...
    def _queue_reason(self, candidate: FeedCandidate) -> str | None:
        ...
        if last_success_at is None:
            return None
        last_seen = parse_timestamp(last_success_at)
        ...
        *,
        ...
        while True:
            try:
                ...
                attempt += 1
                await maybe_await(self.sleep(delay_seconds))
    ...
    def _record_engagement(
        ...
        *,
        ...
        metric_date = self._metric_date(parse_timestamp(created_at))
