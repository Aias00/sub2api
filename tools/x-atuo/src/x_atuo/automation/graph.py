...
from x_atuo.automation.utils import (
    candidate_current_day_reason,
    is_transient_ai_error,
    is_transient_network_error,
    maybe_await,
    parse_created_at_with_original_timezone,
)
...
from x_atuo.core.twitter_client import reply_control_reason
...
AI_MODERATION_METADATA_KEY = _AI_MODERATION_METADATA_KEY
...
def _is_retryable_ai_draft_error(exc: Exception) -> bool:
    if not isinstance(exc, AIProviderError):
        return False
    return is_transient_ai_error(str(exc))
...
def _is_retryable_reply_failure_reason(reason: str | None) -> bool:
    ...
    if not normalized:
        return False
    return is_transient_network_error(normalized)


def _is_moderation_exempt_candidate(candidate: FeedCandidate, exempt_accounts: list[str]) -> bool:
    screen_name = (candidate.screen_name or "").strip().lower()
    return screen_name in {a.strip().lower() for a in exempt_accounts}
...
def _rule_based_reply_style(
    candidate: FeedCandidate | None,
    exempt_accounts: list[str] | None = None,
) -> str | None:
    if candidate is None:
        return None
    screen_name = (candidate.screen_name or "").strip().lower()
    mixed_accounts = {a.strip().lower() for a in (exempt_accounts or ["elonmusk"])}
    if screen_name in mixed_accounts:
        return "mixed"
...
class AutomationGraph:
    ...
    async def prefilter_candidates(self, state: AutomationGraphState) -> AutomationGraphState:
        ...
        removed_reply_restricted: list[str] = []
        for candidate in snapshot.candidates:
            current_day_reason = candidate_current_day_reason(candidate)
...
def _build_runtime_graph(config: AutomationConfig, storage: PolicyHooks | Any, *, proxy: str | None = None) -> AutomationGraph:
    ...
    def hydrate_selected_candidate(candidate: FeedCandidate | None) -> FeedCandidate | None:
        ...
        try:
            ...
            logger.debug("hydration fetch failed for %s: %s", candidate.tweet_id, exc)
            return candidate
    ...
    async def draft_reply(snapshot: WorkflowStateModel):
        ...
        reply_style = _rule_based_reply_style(candidate, config.policies.moderation_exempt_accounts)
    ...
    async def select_candidate(snapshot: WorkflowStateModel):
        ...
        def candidate_reply_disposition(candidate: FeedCandidate) -> str:
            ...
            if candidate.reply_restriction_policy:
                snapshot.log_event(
                    ...
                    reason=(candidate.reply_limit_reason or reply_control_reason(candidate.reply_restriction_policy)),
                )
                return "risky"
            return "eligible"
        ...
        async def moderate_one(candidate: FeedCandidate) -> tuple[str, AIModerationResult | None]:
            ...
            if _is_moderation_exempt_candidate(candidate, config.policies.moderation_exempt_accounts):
                return "ok", None
            ...
            try:
                ...
                if is_transient_ai_error(str(exc)):
...
def build_runtime_graph(config: AutomationConfig, storage: PolicyHooks | Any, *, proxy: str | None = None) -> AutomationGraph:
    return _build_runtime_graph(config, storage, proxy=proxy)
...
def persist_snapshot(storage: Any, snapshot: WorkflowStateModel) -> None:
