...
class AIProviderSettingsLike(Protocol):
    provider: str
    model: str | None
    api_key: str | None
    base_url: str
...
class BaseAIProvider:
    def moderate_candidates(self, candidates: list[CandidateLike]) -> list[AIModerationResult]:
        raise NotImplementedError

    def draft_reply(self, candidate: CandidateLike, context: dict[str, Any] | None = None) -> AIDraftResult:
        raise NotImplementedError

    def classify_reply_style(self, candidate: CandidateLike) -> AIReplyStyleDecision:
        raise NotImplementedError
...
def build_moderation_prompt_payload(candidates: list[CandidateLike]) -> list[dict[str, Any]]:
    return [
...
def build_draft_prompt_payload(candidate: CandidateLike) -> dict[str, Any]:
    return {
...
class OpenAICompatibleProvider(BaseAIProvider):
    ...
    def __init__(self, settings: AIProviderSettingsLike):
...
def build_ai_provider(settings: AIProviderSettingsLike) -> BaseAIProvider | None:
